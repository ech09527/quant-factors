import { listActiveJupyterExecutionKernelBindings } from "./jupyter-execution-db.js";
import {
  getJupyterKernelCapacity,
  JupyterWorkerClient,
  readMaxKernels
} from "./jupyter-async.js";
import { listJupyterMlTaskKernelBindings } from "./ml-task-db.js";
import {
...
function formatMlTaskKernelRow(kernel, binding, executionBinding) {
  ...
  return {
    ...
    linked,
    orphan: !linked && !executionBinding,
    execution: executionBinding
      ? {
          execution_id: executionBinding.execution_id,
          business_type: executionBinding.business_type,
          business_id: executionBinding.business_id,
          execution_status: executionBinding.execution_status,
          task_status: executionBinding.task_status,
          submitted_at: executionBinding.submitted_at
        }
      : null,
    ml_task: linked
      ? {
          ...
          factor_validation_id: binding.factor_validation_id,
...
async function fetchServerMlTaskKernelStatus(server, bindingsByKernelId, executionsByKernelId) {
  ...
  try {
    ...
    const items = kernels
      .map((kernel) =>
        formatMlTaskKernelRow(
          kernel,
          bindingsByKernelId.get(String(kernel?.id ?? "").trim()),
          executionsByKernelId.get(String(kernel?.id ?? "").trim())
        )
      )
      .sort((a, b) => {
    ...
    const linkedCount = items.filter((item) => item.linked || item.execution).length;
...
export async function getMlTaskKernelStatus(db, options = {}) {
  const serverKey = String(options.serverKey ?? "").trim();
  const includeDisabled = options.includeDisabled === true;
  const bindingsByKernelId = await listJupyterMlTaskKernelBindings(db);
  const executionsByKernelId = await listActiveJupyterExecutionKernelBindings(db, serverKey || null);
  ...
  for (const server of servers) {
    const serverExecutions = serverKey
      ? executionsByKernelId
      : await listActiveJupyterExecutionKernelBindings(db, server.key);
    items.push(await fetchServerMlTaskKernelStatus(server, bindingsByKernelId, serverExecutions));
  }
