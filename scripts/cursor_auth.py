...
def build_kernel_cursor_inputs() -> dict[str, str]:
    """Runner 侧：从 GitHub Secrets / 环境 / 本地文件构建 kernel_inputs 凭据字段。"""
    auth_json = os.environ.get("CURSOR_AUTH_JSON", "").strip()
    if auth_json:
        return {"cursor_auth_json": auth_json}

    api_key = os.environ.get("CURSOR_API_KEY", "").strip()
    if api_key:
        return {"cursor_api_key": api_key}

    if DEFAULT_AUTH_PATH.is_file():
        return {
            "cursor_auth_json": DEFAULT_AUTH_PATH.read_text(encoding="utf-8").strip()
        }
